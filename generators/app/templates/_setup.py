...
extension_dir = os.path.join(os.path.dirname(__file__), '<%= parameterized %>')


def install(use_symlink=False, enable=True):
    # Install the livereveal code.
    install_nbextension(extension_dir, symlink=use_symlink,
                        overwrite=use_symlink, user=True)

    if enable:
        cm = ConfigManager()
        cm.update('notebook', {"load_extensions": {"<%= parameterized %>/main": True}})
